...
def test_can_add_class():
    response = app.test_client().post('/classes', json={"id": 0, "class_name": "Test class", "capacity": 2, "start_date": "01-03-2024", "end_date": "3-03-2024"})
...
def test_cannot_add_class_with_invalid_date():
    response = app.test_client().post('/classes', json={"id": 1, "class_name": "Test class", "capacity": 2, "start_date": "invalid", "end_date": "3-03-2024"})
    assert response.status_code == 400
    assert "Invalid date passed" in response.json["errors"]


def test_cannot_add_end_date_before_start():
    response = app.test_client().post('/classes', json={"id": 1, "class_name": "Test class2", "capacity": 2, "start_date": "01-03-2023", "end_date": "3-03-2022"})
    assert response.status_code == 400
 

def test_cannot_add_overlapping_class():
    response = app.test_client().post('/classes', json={"id": 1, "class_name": "Test class2", "capacity": 2, "start_date": "01-03-2024", "end_date": "3-03-2024"})
    assert response.status_code == 400
...
def test_cannot_book_class_over_capacity():
    response = app.test_client().post('/bookings/', json={"id": 2, "date": "01-03-2024"})
    assert response.status_code == 400
