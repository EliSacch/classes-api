...
def test_cannot_book_with_no_classes():
    response = app.test_client().post('/bookings', json={"client_name": "client", "date": "01-01-2023"})
    assert response.status_code == 200
...
def test_can_add_class():
    response = app.test_client().post('/classes', json={"class_name": "Test class", "capacity": 2, "start_date": "01-03-2024", "end_date": "3-03-2024"})
    assert response.status_code == 201


def test_cannot_add_class_with_missing_data():
    response = app.test_client().post('/classes', json={"capacity": 12, "start_date": "01-03-2024", "end_date": "3-03-2024"})
...
def test_cannot_add_class_with_invalid_capacity():
    response = app.test_client().post('/classes', json={"class_name": "Test class", "capacity": "invalid", "start_date": "01-04-2024", "end_date": "3-04-2024"})
    assert response.status_code == 400


def test_cannot_add_class_with_negative_or_zero_capacity():
    response = app.test_client().post('/classes', json={"class_name": "Test class", "capacity": 0, "start_date": "01-04-2024", "end_date": "3-04-2024"})
    assert response.status_code == 400


def test_cannot_add_class_with_invalid_date():
    response = app.test_client().post('/classes', json={"class_name": "Test class", "capacity": 2, "start_date": "invalid", "end_date": "3-03-2024"})
    assert response.status_code == 400
    assert "Invalid date passed" in response.json["errors"]


def test_cannot_add_end_date_before_start():
    response = app.test_client().post('/classes', json={"class_name": "Test class2", "capacity": 2, "start_date": "01-03-2023", "end_date": "3-03-2022"})
    assert response.status_code == 400
 

def test_cannot_add_overlapping_class():
    response = app.test_client().post('/classes', json={"class_name": "Test class2", "capacity": 2, "start_date": "01-03-2024", "end_date": "3-03-2024"})
    assert response.status_code == 400
...
def test_cannot_book_class_with_no_name():
    response = app.test_client().post('/bookings', json={"date": "01-03-2024"})
    assert response.status_code == 400


def test_cannot_book_class_for_a_date_with_no_classes():
    response = app.test_client().post('/bookings', json={"client_name": "client", "date": "01-03-2029"})
    assert response.status_code == 400
    assert response.json['error'] == "Please, choose a valid date."


def test_cannot_book_class_with_invalid_date_format():
    response = app.test_client().post('/bookings', json={"date": "01/01"})
    assert response.status_code == 400
